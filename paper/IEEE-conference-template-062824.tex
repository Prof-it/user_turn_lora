...
% STRUCTURE: Problem → Approach → Key results → Conclusion
% - Problem: User-turn prediction in multi-turn dialogues is underexplored; most work focuses on assistant responses
% - Approach: LoRA fine-tuning of 4 open-source LLMs (1.2B–7B params) on 6,000 training samples from WildChat + SGD
% - Baselines: Base models (no fine-tuning) and GPT-4o-mini (zero-shot + few-shot prompting)
% - Evaluation: BERTScore, BLEURT, perplexity (automated) + blinded human evaluation (369 samples, 3 criteria)
% - Key result: Fine-tuning improves across all metrics; Llama-3B achieves best BERTScore (0.854) and BLEURT (0.393)
% - Key result: Human eval confirms +0.34 avg improvement (4.71 vs 4.37 on 5-point scale)
% - Key result: Larger gains on task-oriented dialogue (+0.45) than open-domain (+0.21)
% - Conclusion: LoRA fine-tuning is a practical, reproducible approach for user-turn prediction
\end{abstract}
...
% ~0.75 page. Hook → Gap → Contribution → Structure.
%
% PARAGRAPH 1 – Context & Motivation
% - Conversational AI has advanced from rule-based to generative transformer models \cite{vaswani2017attention, serban2016hierarchical}
% - Most research optimizes the *assistant's* response; user behavior is modeled with templates or RL agents \cite{shi2019simulators}
% - This imbalance limits evaluation realism and robustness testing of dialogue systems
%
% PARAGRAPH 2 – Research Gap
% - User-turn prediction is harder: user utterances are less constrained, more diverse, context-dependent
% - Recent work (Naous et al., 2025) shows assistant-trained LLMs poorly simulate users \cite{naous2025flipping}
% - Generative user modeling enables: realistic simulation, continuous evaluation, stress testing without human data
%
% PARAGRAPH 3 – Our Approach
% - We investigate LoRA fine-tuning \cite{hu2021lora} of open-source LLMs for next user turn prediction
% - 4 model families (Qwen, Llama, OLMo, LiquidAI) ranging from 1.2B to 7B parameters
% - Cross-domain evaluation: open-domain (WildChat) and task-oriented (Schema-Guided Dialog)
% - Compared against: base models (no fine-tuning) and GPT-4o-mini prompt baselines
%
% PARAGRAPH 4 – Contributions (numbered list)
% 1. First multi-model LoRA study for generative user-turn prediction across domains
% 2. Systematic ablation of LoRA hyperparameters (56 experiments)
% 3. Combined automated (BERTScore, BLEURT, PPL) and blinded human evaluation (369 samples)
% 4. Evidence that fine-tuning yields larger gains on task-oriented than open-domain dialogue
%
% PARAGRAPH 5 – Paper structure
% - Section II: Related Work, Section III: Methodology, Section IV: Experiments, Section V: Results, Section VI: Discussion, Section VII: Conclusion

\section{Related Work}
% ~0.5 page. Three subsections, each 1 short paragraph.
% KEEP COMMENTS until final version.

\subsection{User Simulation and Turn Prediction}
% - Traditional user simulators: rule-based, agenda-based, RL-trained \cite{shi2019simulators}
% - Goal-oriented user simulation with neural models \cite{kim2022multitask, lin2022gentus}
% - LLMs as user proxies: Yoon et al. show LLMs can simulate users but with limitations \cite{yoon2024evaluating}
% - "Flipping the Dialogue" (Naous et al., 2025): first dedicated study on training user LMs \cite{naous2025flipping}
%   → Shows assistant-trained models are poor user simulators; dedicated training needed
%   → Our work extends this by: multiple model families, LoRA (not full fine-tuning), cross-domain analysis
...
% - Full fine-tuning of LLMs is prohibitively expensive for academic settings
% - LoRA \cite{hu2021lora}: injects low-rank matrices, freezes pretrained weights, reduces trainable params by >99%
% - QLoRA \cite{dettmers2023qlora}: combines 4-bit quantization with LoRA for further memory savings
% - Enables controlled study of behavioral adaptation without full model relearning
% - Practical for reproducible research on consumer/academic hardware

Full fine-tuning of large language models is prohibitively expensive for many research settings, motivating parameter-efficient alternatives. LoRA~\cite{hu2021lora} freezes pretrained weights and injects trainable low-rank decomposition matrices into transformer layers, reducing trainable parameters by over 99\% while matching full fine-tuning quality. The key insight is that weight updates during adaptation reside on a low intrinsic dimension, so very low ranks suffice. QLoRA~\cite{dettmers2023qlora} extends this by quantizing the frozen model to 4-bit precision and introducing paged optimizers for memory spikes, enabling fine-tuning of models up to 65B parameters on a single 48\,GB GPU. These methods are particularly relevant for our setting: they allow controlled study of behavioral adaptation---shifting a model from assistant-style to user-style generation---without full retraining, and they make the experiments reproducible on academic hardware.

\subsection{Evaluation of Generated Dialogue}
% - Perplexity: token-level predictability, measures fluency
% - BERTScore: contextual embedding similarity, captures paraphrases \cite{zhang2020bertscore}
% - BLEURT: learned metric trained on human judgments, robust to distribution shifts \cite{sellam2020bleurt}
% - Automated metrics alone insufficient → human evaluation needed for conversational plausibility
% - Our evaluation combines all three automated metrics + blinded human study (Relevance, Coherence, Naturalness)
...
\section{Methodology}
% ~1 page. Compact, reproducible.

\subsection{Task Definition}
% - Input: conversation history $C = \{u_1, a_1, u_2, a_2, \ldots, a_{t-1}\}$
% - Output: predicted next user utterance $\hat{u}_t$
% - Generative formulation: model generates free-form text, not classification
% - This differs from assistant modeling: user turns are less structured, more diverse, often terse

Given a multi-turn conversation history $C = (u_1, a_1, u_2, a_2, \ldots, a_{t-1})$ between a user and an assistant, the task is to generate the next user utterance $\hat{u}_t$. We frame this as a conditional language generation problem: the model receives the full context $C$ formatted as a chat template and produces $\hat{u}_t$ autoregressively. During training, only the tokens of the target user turn are supervised; all context tokens are masked (label $= -100$). This formulation differs from standard assistant modeling: user turns are typically shorter, lack the instructional structure of assistant responses, and span a wider range of communicative intents---from terse confirmations to complex multi-part requests.

\subsection{Datasets}
% - Two complementary sources to test cross-domain generalization:
%   1. WildChat-1M \cite{zhao2024wildchat}: real ChatGPT interactions, open-domain, diverse topics/languages
%   2. Schema-Guided Dialog \cite{rastogi2020sgd}: task-oriented, structured intents, multi-domain services
% - Filtering: English only, ≥3 turns, removed toxic/empty conversations
% - Split: 6,000 train (3,000 per domain), 400 eval (200 per domain), seed=42
...
\subsection{Models}
% - 4 open-source instruction-tuned LLMs spanning 1.2B–7B parameters
% - Chosen to test whether LoRA gains generalize across architectures and scales
% - All use chat-template formatting with system/user/assistant roles
% - Qwen2.5-3B \cite{qwen2025}: strong multilingual model, ChatML format
% - Llama-3.2-3B: Meta's latest small model, Llama-style tokens
% - OLMo-3-7B: Allen AI's open model, largest in our study (tests if bigger = better)
% - LiquidAI LFM2.5-1.2B: smallest model, used for ablation (fastest iteration)
% - GPT-4o-mini: proprietary prompt baseline (no fine-tuning, API-only)
...
% - Hardware: NVIDIA A100-SXM4-40GB on GCP (upgraded from Colab T4 used in prior work)
% - 4-bit quantization (QLoRA) with bfloat16 compute dtype
% - Effective batch size: 64 (batch=4, gradient_accumulation=16)
% - Optimizer: paged_adamw_8bit with cosine LR scheduler
% - Gradient checkpointing enabled; max context length 4096 tokens
% - Training supervised only on target user turn tokens (context tokens masked with -100)
% - Modular Python pipeline (src/*.py) with Docker for reproducibility
% - All models trained with identical config from ablation (applied uniformly)
...
% - Determined via systematic two-stage ablation (Section IV-C)
% - Key difference from thesis: thesis used r=16, α=32, dropout=0.1, lr=8e-4 (manual tuning)
% - New ablation found: α is the most impactful parameter; rank has minimal effect
% - r=8 performs as well as r=64, suggesting user-turn adaptation has low intrinsic rank
% - Dropout=0.0 optimal (no regularization needed at this data scale)
...
% - Perplexity (PPL): token-level fluency; lower = model assigns higher probability to ground truth tokens
% - BERTScore F1 \cite{zhang2020bertscore}: contextual embedding similarity (0–1); captures paraphrases and synonyms
% - BLEURT \cite{sellam2020bleurt}: learned metric trained on human judgments; robust to surface-level variation
% - Human evaluation: Relevance, Coherence, Naturalness on 1–5 Likert scale (blinded, randomized labels A–D)
% NOTE: PPL not directly comparable for GPT-4o-mini (API model, no logprob access for same formulation)
...
% 1. Base model: same architecture, no fine-tuning (measures what the pretrained model already knows)
% 2. Zero-shot prompt (GPT-4o-mini): system instruction + conversation context, no examples
% 3. Few-shot prompt (GPT-4o-mini): same + 3 fixed exemplar (context → user turn) pairs from training set
% - Prompt baselines answer: "Can a stronger model solve this without any training, just by prompting?"
% - Temperature 0.4, max 256 tokens for prompt baselines
...
% - Conducted on LiquidAI/LFM2.5-1.2B (smallest model, fastest iteration)
% - Two-stage design: Stage 1 = LoRA hyperparams (24 experiments), Stage 2 = training hyperparams (32 experiments)
% - Total: 56 experiments, ~4 hours on A100-40GB
% - Best config: r=8, α=64, dropout=0.0, lr=2e-4, epochs=3, warmup=0.1
% - This config was then applied to all 4 model families
...
% - Tested temperatures: 0.3, 0.4, 0.5, 0.6, 0.7 on all fine-tuned models
% - Optimal: 0.3 for Qwen, Llama, LiquidAI; 0.6 for OLMo
% - Lower temperature generally improves BERTScore (more deterministic generation)
...
\section{Results}
% ~1.5 pages. Tables + brief interpretation.
...
% TODO: Add temperature sweep figure
% \begin{figure}[h]
% \centering
% \includegraphics[width=\columnwidth]{figures/temperature_sweep_comparison.png}
% \caption{Temperature sweep results across models}
% \label{fig:temperature}
% \end{figure}

\subsection{Human Evaluation}

Table~\ref{tab:human-eval} presents the blinded human evaluation results. Fine-tuning improves the average rating from 4.37 to 4.71 (+0.34 on a 5-point scale), with the largest gain on naturalness (+0.37). GPT-4o-mini scores highest overall (4.80--4.82), though the gap to the fine-tuned Qwen-3B is modest. Table~\ref{tab:human-eval-dataset} breaks down results by domain: task-oriented dialogue (SGD) benefits substantially more from fine-tuning (+0.45) than open-domain conversation (WildChat, +0.21), suggesting that the structured intent patterns in SGD are easier to learn from limited data.

\begin{table}[h]
\centering
\caption{Human Evaluation Results (n=369, 5.77\% of corpus)}
\label{tab:human-eval}
\begin{tabular}{lcccc}
\toprule
\textbf{Condition} & \textbf{Rel.} & \textbf{Coh.} & \textbf{Nat.} & \textbf{Avg.} \\
\midrule
Baseline (Qwen-3B) & 4.34 & 4.34 & 4.44 & 4.37 \\
\textbf{Fine-tuned (Qwen-3B)} & 4.64 & 4.68 & 4.81 & \textbf{4.71} \\
Zero-shot (GPT-4o-mini) & 4.78 & 4.82 & 4.85 & 4.82 \\
Few-shot (GPT-4o-mini) & 4.77 & 4.80 & 4.83 & 4.80 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Human Evaluation by Dataset}
\label{tab:human-eval-dataset}
\begin{tabular}{l|cc|cc}
\toprule
& \multicolumn{2}{c|}{\textbf{Schema-Guided}} & \multicolumn{2}{c}{\textbf{WildChat}} \\
\textbf{Condition} & Avg. & $\Delta$ & Avg. & $\Delta$ \\
\midrule
Baseline & 4.50 & -- & 4.22 & -- \\
Fine-tuned & 4.95 & +0.45 & 4.43 & +0.21 \\
\bottomrule
\end{tabular}
\end{table}

\section{Discussion}
% ~0.75 page. Interpret results, compare to thesis, acknowledge limitations.
%
% PARAGRAPH 1 – Main finding: Fine-tuning works
% - LoRA fine-tuning consistently improves user-turn prediction across 3 of 4 model families
% - Largest PPL reductions: LiquidAI-1.2B (409→9.4, 97.7% reduction), Llama-3B (100→11.2, 88.9%)
% - BERTScore/BLEURT improvements confirm semantic quality, not just fluency
% - Human eval corroborates: +0.34 avg on 5-point scale (4.71 vs 4.37)
...
% PARAGRAPH 2 – Domain sensitivity
% - Task-oriented (SGD) shows larger gains than open-domain (WildChat) in human eval (+0.45 vs +0.21)
% - Consistent with thesis finding: structured intents in TOD are easier to learn
% - Open-domain user behavior is more diverse, harder to model with limited data
% - This suggests domain-specific fine-tuning may be more effective than general-purpose
...
% PARAGRAPH 3 – OLMo anomaly
% - OLMo-7B is the only model where fine-tuning *decreases* BERTScore (0.813→0.798) and BLEURT (0.285→0.229)
% - PPL still improves (11.1→9.0), suggesting the model learns fluency but loses semantic alignment
% - Possible explanations: (1) larger model may need different LoRA config, (2) OLMo architecture differences
% - The ablation was conducted on LiquidAI-1.2B; optimal config may not transfer to all architectures
...
% PARAGRAPH 4 – Prompt baselines vs fine-tuning
% - GPT-4o-mini (zero-shot: 0.841 BERTScore, few-shot: 0.847) matches or exceeds fine-tuned Qwen (0.846)
% - Human eval: GPT-4o-mini (4.80–4.82) > fine-tuned Qwen (4.71) > baseline Qwen (4.37)
% - However: GPT-4o-mini is a much larger proprietary model; fine-tuning open-source 3B models is more practical
% - Fine-tuned Llama-3B (0.854 BERTScore, 0.393 BLEURT) actually exceeds GPT-4o-mini on automated metrics
...
% PARAGRAPH 5 – Ablation insights (different from thesis)
% - Thesis used r=16, α=32, dropout=0.1, lr=8e-4 on Colab T4 with 800 samples
% - Systematic ablation (56 experiments) found: r=8, α=64, dropout=0.0, lr=2e-4 is optimal
% - Key insight: α is the most impactful LoRA parameter; rank has minimal effect (r=8 ≈ r=64)
% - This suggests user-turn adaptation resides on a very low-rank subspace
...
% PARAGRAPH 6 – Temperature
% - Optimal temperature is 0.3 for most models (more deterministic = better semantic match)
% - OLMo prefers 0.6 (higher diversity needed to compensate for training issues)
...
% - Single rater for human evaluation (no inter-rater reliability)
% - Ablation on smallest model only; may not generalize to all architectures (see OLMo)
% - English-only evaluation
% - No interactive/multi-turn rollout evaluation
% - BERTScore/BLEURT measure similarity to one ground truth; multiple valid user turns exist
...
\section{Conclusion}
% ~0.25 page. Concise summary + future work.
%
% PARAGRAPH 1 – Summary
% - We investigated LoRA fine-tuning for next user turn prediction across 4 open-source LLMs
% - Fine-tuning on 6,000 samples yields consistent improvements in fluency (PPL) and semantic quality (BERTScore, BLEURT)
% - Blinded human evaluation (369 samples) confirms improvements in relevance, coherence, and naturalness
% - Task-oriented dialogue benefits more from fine-tuning than open-domain conversation
% - Fine-tuned Llama-3B achieves the best overall automated metrics (BERTScore 0.854, BLEURT 0.393)
%
% PARAGRAPH 2 – Practical implications
% - LoRA enables user-turn modeling on academic hardware (A100 or even consumer GPUs with QLoRA)
% - Open-source fine-tuned models can match proprietary API models (GPT-4o-mini) on automated metrics
% - Modular pipeline (src/*.py + Docker) ensures full reproducibility
%
% PARAGRAPH 3 – Future work
% - Multi-turn rollout evaluation (generate full conversations, not single turns)
% - Per-model ablation to address OLMo regression
% - Larger training sets and multilingual evaluation
% - Preference-based training (DPO/RLHF) for user-turn quality
% - Integration with dialogue system evaluation pipelines
